...
- Cancel-on-refund [CONCLUÍDO contra o stub]: webhooks orders/cancelled + refunds/create → cancelInvoiceForOrder (invoice-resolver.server.ts) via adapter.cancelInvoice. Só cancela em cancelamento/reembolso INTEGRAL (refunds/create consulta displayFinancialStatus/totalRefunded via Admin GraphQL); parcial → só sinaliza partialRefundAt (NÃO cancela). Por estado: AUTHORIZED→CANCELLED (update condicional, cancelledAt+protocoloCancelamento); PROCESSING→grava cancelRequestedAt e o resolver encadeia ao autorizar; REJECTED/ERROR→no-op fiscal; CANCELLED→no-op. Recusa da SEFAZ mantém AUTHORIZED + mensagemSefaz. Migration 20260630_add_invoice_cancel_fields (cancelRequestedAt/partialRefundAt/protocoloCancelamento). FLAGs p/ contador: (1) texto da justificativa de cancelamento; (2) janela de >24h varia por UF — só sinalizamos o risco e tentamos mesmo assim; (3) refund parcial = nota de devolução (fluxo futuro, não implementado). As assinaturas novas de webhook (orders/cancelled, refunds/create) só passam a valer após redeploy/reinstall do app (shopify app deploy + reinstalar/atualizar scopes).
